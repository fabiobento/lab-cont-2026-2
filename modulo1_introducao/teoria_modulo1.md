...
Como $G(s)$ vale para toda entrada, obtemos a saída para uma entrada específica por $Y(s) = G(s)U(s)$. E $U(s)$? Pela definição, $U(s) = \int_0^\infty e^{-st}u(t)dt$ — **Não se assute! Na verdade, não vamos calcular essa integral**: usamos uma **tabela de Transformadas de Laplace**.
